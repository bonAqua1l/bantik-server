...
function useCreateProduct() {
  ...
  const [productsColorsList, setProductsColorsList] = React.useState<ProductsOutgoingTypes.Color[] | undefined>(undefined)
  const [submitted, setSubmitted] = React.useState(false)
  const [isCreated, setIsCreated] = React.useState(false)
  const [api, contextHolder] = notification.useNotification()

  const createProduct = React.useCallback(async (data: ProductsOutgoingTypes.FormProduct) => {
    setSubmitted(true)
    try {
  ...
  const ProductsColorsGET = React.useCallback(async () => {
    try {
      const response = await ProductsOutgoing.API.Create.getProductsColors()
